...
# -*- coding: UTF-8 -*-
'''
@Project ：SchoolCardManageSystem
@File    ：main.py
@IDE     ：pycharm
@Author  ：162130316jh
...
from ui import ui_main, ui_index, ui_login, ui_reset, ui_register, ui_forget, ui_loss, ui_manage, ui_select, \
...
def connect_mysql():  # 连接数据库
    try:
        ...
        QMessageBox(QMessageBox.Warning, '错误', '未检测到数据库').exec_()
        return False
...
def click_set_btn():
    ...
    if len(reset_window.password_edit.text()) != 0 and len(reset_window.check_edit.text()) != 0 and len(
            reset_window.sid_edit.text()) != 0:  #输入为空
        if reset_window.password_edit.text() == reset_window.check_edit.text():  #是否一致
            try:
...
def click_register_btn():
    ...
    if not user.isdecimal():
        QMessageBox(QMessageBox.Warning, '警告', '学号/工号只含有数字！').exec_()
        cur.close()
        return
    if user == '' or password == '':
        ...
        if password == check_pwd:
            ...
            for i in list_info:
                ...
                sql = f"insert into user_info(user,pwd,type,cardHolder) values ('{user}', '{password}', 1,0);"
                cur.execute(sql)
                conn.commit()
    ...
    if is_loss:
        ...
        if len(sid) == 0 or len(name) == 0 or len(phone) == 0 or len(card_id) == 0 or len(reason) == 0:
            ...
            if info[0] == (int(sid), name, phone, int(card_id)):
                ...
                sql = f"update card_info set cardlock = 1 where cardId = '{card_id}'"
                cur.execute(sql)
                conn.commit()
                try:
...
def flush_all_btn():
    ...
    cur.execute(sql)
    info = cur.fetchall()
    select_all_window.lineEdit.setText("")
    lists = []
    user_type = ''
    card_lock = ''
    for i in info:
...
def info_search():
    text=select_all_window.lineEdit.text()
    row_num=select_all_window.info_table.rowCount()
    if text=="":
        for i in range(row_num):
            select_all_window.info_table.setRowHidden(i,False)
    else:
        item = select_all_window.info_table.findItems(text, Qt.MatchContains);
        for i in range(row_num):
            select_all_window.info_table.setRowHidden(i,True)
        if item!=None:
            for i in range(len(item)):
                select_all_window.info_table.setRowHidden(item[i].row(), False)
...
def click_loss_and_logout_btn():
    ...
    loss_and_logout_window.loss_table.resizeColumnsToContents()
    loss_and_logout_window.loss_table.setColumnWidth(0,140)
    loss_and_logout_window.loss_table.setColumnWidth(6, 180)
    loss_and_logout_window.loss_table.setEditTriggers(QAbstractItemView.NoEditTriggers)
    for i in range(len(lists)):
        for j in range(7):
            if j==1:
                if lists[i][j]==1:
                    loss_and_logout_window.loss_table.setItem(i, j, QTableWidgetItem("学生"))
                else:
                    loss_and_logout_window.loss_table.setItem(i, j, QTableWidgetItem("教职工"))
            else:
...
def click_statis(): #消费数据统计
    con = connect_mysql()
    cur = con.cursor()
    sql = "select sum(money) from cardhistory_info group by operation"
    cur.execute(sql)
    sta = cur.fetchall()
    l = []
    l.append(sta[0][0])
...
def click_spend_btn():
    ...
    if (balance - 10) > 0:
        ...
        cur.execute(sql)
        conn.commit()
        sql = f"insert into cardhistory_info (cardId,opear_time,money,operation,info)values('{cardId}', '{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}', -10, 1, '模拟消费')"
        cur.execute(sql)
        conn.commit()
...
if __name__ == '__main__':
    ...
    # changeCardInfo 修改校园卡信息界面
    change_card_info_window.change_btn.clicked.connect(click_change_info_btn)  # 修改校园卡信息界面点击修改
    ...
    manage_window.back_btn_2.clicked.connect(click_statis) #数据统计
    # selectAll 查询所有界面
    ...
    select_all_window.lineEdit.textChanged.connect(info_search) #查找
    ...
    user_info_window.change_btn.clicked.connect(click_change_bth)  # 用户信息界面点击校园卡
    ...
    loss_and_logout_window.loss_table.doubleClicked.connect(double_click_loss_table)  #解挂
    loss_and_logout_window.logout_table.doubleClicked.connect(double_click_logout_table)  #注销
    apply_stylesheet(app, theme='light_cyan_500.xml') # 配置样式主题
    ''' 关闭程序，释放资源 '''
    sys.exit(app.exec_())
